...
#set up how the script should run; change these variables to load only specific data
#say from a certain provider, or a certain month etc
#global variables - directory / provider / month variables
prov = "att" #att, sprint, tmobile, vz
month = "9"
myDir = "/Users/feomike/documents/analysis/2013/mmba/data/" + prov + \
        "/json/" + "20130" + month + "/"
#myDir = "/Users/feomike/documents/analysis/2013/mmba/data/" + prov + "/test/"
...
def myLocation(myData, theStr):
	...
	while l > 0:
		if myData["metrics"][l]["type"] == "location":
			myLon = myData["metrics"][l]["longitude"]
			myLat = myData["metrics"][l]["latitude"]
		l = l - 1
	#append the new values to the end of the existing string, 
	theStr = theStr + "," + str(myLon) + "," + str(myLat)
	return(theStr)

#insert into raw table
def insertRaw(theStr, myCur):
	#set up python connection
	#write insert string
	#insert
	#set up the connection to the database
	theSQL = "INSERT INTO " + schema + ".test VALUES (" 
	theSQL = theSQL + theStr + "); Commit;"
